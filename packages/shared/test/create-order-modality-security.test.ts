...
test("create-order revalida entrega e retirada no instante do checkout", () => {
  assert.match(compact, /select\("pickup_enabled,clickfood_delivery_enabled,own_delivery_enabled"\)/);
  assert.match(compact, /body\.deliveryType==="DELIVERY"[\s\S]*DELIVERY_DISABLED/);
  assert.match(compact, /body\.deliveryType==="PICKUP"[\s\S]*PICKUP_DISABLED/);

  const settingsCheck = compact.indexOf('.select("pickup_enabled,clickfood_delivery_enabled,own_delivery_enabled")');
  const quoteCheck = compact.indexOf('if(!body.addressId||!body.deliveryQuoteId)');
