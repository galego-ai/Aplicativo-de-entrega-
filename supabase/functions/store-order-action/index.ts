...
type ActionBody = { orderId:string; action:"ACCEPT"|"REJECT"|"START_PREPARING"|"MARK_READY"|"CANCEL"; reason?:string };
...
const allowedRolesByAction:Record<ActionBody["action"],string[]>={ACCEPT:["OWNER","MANAGER"],REJECT:["OWNER","MANAGER"],START_PREPARING:["OWNER","MANAGER","KITCHEN"],MARK_READY:["OWNER","MANAGER","KITCHEN","EXPEDITION"],CANCEL:["OWNER","MANAGER"]};

async function requestRefund(req:Request,orderId:string,reason:string){
 const auth=req.headers.get("Authorization")??"";if(!auth)return{ok:false,data:{error:"AUTH_REQUIRED"}};
 ...
 let data:any={};try{data=await response.json()}catch{}return{ok:response.ok,data};
}
...
export default{fetch:withSupabase({auth:"user"},async(req,ctx)=>{
 ...
 if(body.action==="CANCEL"){expected=order.status;next="CANCELLED";if(!["WAITING_STORE","ACCEPTED","PREPARING","READY","WAITING_DRIVER","DRIVER_ASSIGNED"].includes(expected))return Response.json({error:"ORDER_CANNOT_BE_CANCELLED"},{status:409});}
 else{[expected,next]=transitionByAction[body.action];if(order.status!==expected)return Response.json({error:"ORDER_STATUS_CHANGED",currentStatus:order.status},{status:409});}
 ...
 if(["REJECTED","CANCELLED"].includes(next)){
  ...
  if(["PAID","PARTIALLY_REFUNDED"].includes(order.payment_status)){
   const refund=await requestRefund(req,order.id,body.reason?.trim()||"Pedido cancelado pela loja");
   return Response.json({order:updatedOrder,dispatchRequired:false,refundRequired:true,refundPending:!refund.ok||!["COMPLETED","FAILED"].includes(String(refund.data?.refundStatus??"")),refundStatus:refund.data?.refundStatus??"PENDING",refundError:refund.ok?null:(refund.data?.error??"EFI_PIX_REFUND_FAILED")});
  }
 }
