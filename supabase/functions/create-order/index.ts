...
export default {
  fetch: withSupabase({ auth: "user" }, async (req, ctx) => {
    ...
    if (!body.storeId || !body.deliveryType || !allowedPaymentMethods.has(body.paymentMethod) || !Array.isArray(body.items) || body.items.length === 0) {
    ...
    const { data: store, error: storeError } = await ctx.supabaseAdmin
      .from("stores")
      .select("id,status,minimum_order")
      .eq("id", body.storeId)
      .maybeSingle();
    ...
    if (body.deliveryType === "DELIVERY") {
      if (!body.addressId || !body.deliveryQuoteId) {
      ...
      const { data: deliverySettings } = await ctx.supabaseAdmin
        ...
        .eq("store_id", body.storeId)
        .maybeSingle();
      if (deliverySettings && !deliverySettings.pickup_enabled) {
    ...
    if (requestedProductIds.some((id) => typeof id !== "string" || !id)) {
    ...
    if (!products || products.length !== requestedProductIds.length) {
      return Response.json({ error: "PRODUCT_NOT_FOUND" }, { status: 409 });
    }
    ...
    if (groupsResult.error || optionsResult.error) {
      return Response.json({ error: "OPTION_LOOKUP_FAILED" }, { status: 500 });
    }
    ...
    for (const requested of body.items) {
      ...
      for (const groupId of allowedGroups) {
        ...
        if (selectedCount > Number(group.maximum_choices)) {
          return Response.json({ error: "TOO_MANY_OPTIONS", groupId }, { status: 409 });
        }
      }
    ...
    if (body.couponCode?.trim()) {
      ...
      if (!coupon || (coupon.store_id && coupon.store_id !== body.storeId)) {
        return Response.json({ error: "COUPON_INVALID" }, { status: 409 });
      }
      ...
      if (subtotal < Number(coupon.minimum_order)) {
        return Response.json({ error: "COUPON_MINIMUM_NOT_REACHED" }, { status: 409 });
      }
      ...
      if (coupon.max_uses != null && (globalUses ?? 0) >= Number(coupon.max_uses)) {
        return Response.json({ error: "COUPON_LIMIT_REACHED" }, { status: 409 });
      }
      if (coupon.max_uses_per_customer != null && (customerUses ?? 0) >= Number(coupon.max_uses_per_customer)) {
        return Response.json({ error: "COUPON_CUSTOMER_LIMIT_REACHED" }, { status: 409 });
      }
      ...
      for (const rule of rules ?? []) {
        if (rule.rule_type === "FIRST_ORDER") {
          ...
          // Regras dependentes de horário/dia só serão habilitadas quando a timezone da loja estiver configurada.
