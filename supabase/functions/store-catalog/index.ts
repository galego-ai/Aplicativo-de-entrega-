...
export default{fetch:withSupabase({auth:"user"},async(req,ctx)=>{
 ...
 const{data:stores,error}=await ctx.supabaseAdmin.from("stores").select("id,name,slogan,description,logo_url,cover_url,primary_color,secondary_color,minimum_order,average_preparation_time,orders_paused,city_id,cities(timezone)").eq("status","ACTIVE").order("name").limit(100);
 ...
 const result=(stores??[]).map((s:any)=>{const city=Array.isArray(s.cities)?s.cities[0]:s.cities;const timezone=String(city?.timezone??"America/Sao_Paulo");const delivery:any=deliveryByStore.get(String(s.id));const ordersPaused=Boolean(s.orders_paused);return{id:s.id,name:s.name,slogan:s.slogan,description:s.description,logo_url:s.logo_url,cover_url:s.cover_url,primary_color:s.primary_color??"#F4C400",secondary_color:s.secondary_color??"#111111",minimum_order:Number(s.minimum_order),average_preparation_time:Number(s.average_preparation_time),timezone,orders_paused:ordersPaused,open_now:!ordersPaused&&isOpen(byStore.get(s.id)??[],timezone),pickup_enabled:Boolean(delivery?.pickup_enabled??true),clickfood_delivery_enabled:Boolean(delivery?.clickfood_delivery_enabled??true),own_delivery_enabled:Boolean(delivery?.own_delivery_enabled??false),max_radius_km:delivery?.max_radius_km==null?null:Number(delivery.max_radius_km)};});
