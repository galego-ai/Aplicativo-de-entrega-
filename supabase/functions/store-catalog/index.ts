...
export default{fetch:withSupabase({auth:"user"},async(req,ctx)=>{
 ...
 const{data:stores,error}=await ctx.supabaseAdmin.from("stores").select("id,name,slogan,description,logo_url,cover_url,primary_color,secondary_color,phone,whatsapp,instagram,address_line,neighborhood,minimum_order,average_preparation_time,city_id,cities(timezone)").eq("status","ACTIVE").order("name").limit(100);
 ...
 const result=(stores??[]).map((s:any)=>{const city=Array.isArray(s.cities)?s.cities[0]:s.cities;const timezone=String(city?.timezone??"America/Sao_Paulo");const delivery:any=deliveryByStore.get(String(s.id));return{id:s.id,name:s.name,slogan:s.slogan,description:s.description,logo_url:s.logo_url,cover_url:s.cover_url,primary_color:s.primary_color??"#F4C400",secondary_color:s.secondary_color??"#111111",phone:s.phone,whatsapp:s.whatsapp,instagram:s.instagram,address_line:s.address_line,neighborhood:s.neighborhood,minimum_order:Number(s.minimum_order),average_preparation_time:Number(s.average_preparation_time),timezone,open_now:isOpen(byStore.get(s.id)??[],timezone),pickup_enabled:Boolean(delivery?.pickup_enabled??true),clickfood_delivery_enabled:Boolean(delivery?.clickfood_delivery_enabled??true),own_delivery_enabled:Boolean(delivery?.own_delivery_enabled??false),max_radius_km:delivery?.max_radius_km==null?null:Number(delivery.max_radius_km)};});
