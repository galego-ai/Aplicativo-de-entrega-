...
type Body=
 | {action:"REQUEST";storeId:string;amount:number;method:"PIX"|"BANK_TRANSFER"|"OTHER";destinationValue:string}
 | {action:"CANCEL";payoutId:string}
 | {action:"ADMIN_STATUS";payoutId:string;status:"APPROVED"|"PROCESSING"|"PAID"|"FAILED"|"REJECTED";notes?:string;providerId?:string};

function errorCode(message?:string){for(const code of ["INVALID_AMOUNT","INVALID_METHOD","DESTINATION_REQUIRED","STORE_MANAGER_REQUIRED","INSUFFICIENT_AVAILABLE_BALANCE","PAYOUT_NOT_FOUND","INVALID_RECIPIENT","INVALID_PAYOUT_TRANSITION"])if(message?.includes(code))return code;return "PAYOUT_OPERATION_FAILED";}
...
export default{fetch:withSupabase({auth:"user"},async(req,ctx)=>{
 ...
 if(body.action==="ADMIN_STATUS"){
  ...
  const{data,error}=await ctx.supabaseAdmin.schema("private").rpc("review_store_payout_atomic",{p_payout_id:body.payoutId,p_target_status:body.status,p_actor_id:userId,p_notes:body.notes??null,p_provider_id:body.providerId??null});
  if(error)return Response.json({error:errorCode(error.message)},{status:409});return Response.json({payout:data});
 }
