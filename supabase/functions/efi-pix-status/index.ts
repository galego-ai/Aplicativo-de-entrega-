...
export default{fetch:withSupabase({auth:"user"},async(req,ctx)=>{
 ...
 const client=httpClient();try{const access=await token(client);const res=await fetch(`${baseUrl()}/v2/cob/${encodeURIComponent(charge.txid)}`,{headers:{Authorization:`Bearer ${access}`},client} as any);const data=await res.json();if(!res.ok)return Response.json({error:"EFI_STATUS_FAILED"},{status:502});
