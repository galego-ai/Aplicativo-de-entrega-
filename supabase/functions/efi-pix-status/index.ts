...
export default{fetch:withSupabase({auth:"user"},async(req,ctx)=>{
  ...
  const{data:order}=await ctx.supabaseAdmin.from("orders").select("id,customer_id,payment_status").eq("id",orderId).maybeSingle();
  ...
  const{data:charge}=await ctx.supabaseAdmin.from("efi_pix_charges").select("txid,status,brcode,qr_image,visualization_url,expires_at").eq("order_id",orderId).maybeSingle();
  if(!charge)return Response.json({error:"EFI_PIX_CHARGE_NOT_FOUND"},{status:404});
  ...
  try{
    ...
    const res=await fetch(`${baseUrl()}/v2/cob/${encodeURIComponent(charge.txid)}`,{headers:{Authorization:`Bearer ${access}`},client} as any);
    let data:any={};try{data=await res.json();}catch{}
    if(!res.ok)return Response.json({error:"EFI_STATUS_FAILED",charge},{status:502});

    if(String(data?.status)==="CONCLUIDA"&&Array.isArray(data?.pix)&&data.pix.length){
      const pix=data.pix[0];const value=Number(String(pix?.valor??"0").replace(",","."));const paidAt=pix?.horario?new Date(pix.horario).toISOString():new Date().toISOString();
      const{error}=await ctx.supabaseAdmin.rpc("complete_efi_pix_payment_atomic",{p_txid:charge.txid,p_end_to_end_id:String(pix?.endToEndId??"")||null,p_paid_amount:value,p_paid_at:paidAt,p_payload:pix});
      if(error)return Response.json({error:String(error.message).includes("AMOUNT_MISMATCH")?"EFI_PIX_AMOUNT_MISMATCH":"PAYMENT_CONFIRMATION_FAILED"},{status:409});
      return Response.json({paid:true,status:"CONCLUIDA",charge:{...charge,status:"PAID"}});
    }

    return Response.json({paid:false,status:String(data?.status??charge.status),charge});
  }catch{return Response.json({error:"EFI_STATUS_FAILED",charge},{status:502});}
