...
export default {
  fetch: withSupabase({ auth: "user" }, async (req, ctx) => {
    ...
    if (body.action === "CONFIRM_PICKUP" || body.action === "CONFIRM_DELIVERY") {
      ...
      if (body.code !== expected) {
        await ctx.supabaseAdmin.from("audit_logs").insert({
          actor_id: userId,
          action: "DELIVERY_CODE_REJECTED",
          entity_type: "delivery",
          entity_id: delivery.id,
          after_data: { kind: body.action === "CONFIRM_PICKUP" ? "PICKUP" : "DELIVERY" },
        });
    ...
    if (body.action === "CONFIRM_PICKUP") {
      ...
      const transition = body.action === "START_TO_STORE"
        ? ["DRIVER_ASSIGNED", "DRIVER_TO_STORE"]
        : body.action === "ARRIVED_STORE"
          ? ["DRIVER_TO_STORE", "DRIVER_AT_STORE"]
          : ["DRIVER_TO_CUSTOMER", "DRIVER_AT_CUSTOMER"];
      rpcName = "transition_delivery_atomic";
      params = {
        p_delivery_id: delivery.id,
        p_expected_status: transition[0],
        p_next_status: transition[1],
