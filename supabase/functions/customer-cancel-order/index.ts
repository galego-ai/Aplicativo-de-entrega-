...
async function requestRefund(req:Request,orderId:string,reason:string){
  ...
  const response=await fetch(`${Deno.env.get("SUPABASE_URL")}/functions/v1/payment-refund`,{method:"POST",headers:{Authorization:auth,"Content-Type":"application/json"},body:JSON.stringify({orderId,reason})});
...
export default {
  fetch: withSupabase({ auth: "user" }, async (req, ctx) => {
    ...
    const { data: order, error: lookupError } = await ctx.supabaseAdmin.from("orders").select("id,customer_id,store_id,status,payment_status").eq("id", body.orderId).maybeSingle();
    ...
    const { data: updatedOrder, error: transitionError } = await ctx.supabaseAdmin.rpc("transition_order_atomic", {p_order_id: order.id,p_expected_status: order.status,p_next_status: "CANCELLED",p_actor_id: userId,p_reason: reason.slice(0, 500)});
    ...
    if (delivery && !delivery.driver_id && !["DELIVERED", "DELIVERY_CANCELLED"].includes(delivery.status)) await Promise.all([ctx.supabaseAdmin.from("deliveries").update({ status: "DELIVERY_CANCELLED", updated_at: new Date().toISOString() }).eq("id", delivery.id),ctx.supabaseAdmin.from("delivery_offers").update({ status: "EXPIRED", responded_at: new Date().toISOString() }).eq("delivery_id", delivery.id).eq("status", "PENDING")]);
