...
replace_once(
    ...
    "sincronização modalidade",
)

old_open = '''  async function openStore(store:Store){
...
new_open = '''  async function openStore(store:Store){
...
replace_once(old_open, new_open, "carregamento do cardápio")
...
replace_once(
    ...
    "proteção de modalidade no pedido",
)
...
old_header = '''      {selectedStore.cover_url?<Image source={{uri:selectedStore.cover_url}} style={styles.storeCover}/>:<View style={styles.storeCoverFallback}><Text style={styles.storeCoverFallbackText}>CLICK-FOOD</Text></View>}
...
new_header = '''      {selectedStore.cover_url?<Image source={{uri:selectedStore.cover_url}} style={styles.storeCover}/>:<View style={[styles.storeCoverFallback,{backgroundColor:selectedStore.secondary_color||"#111"}]}><Text style={[styles.storeCoverFallbackText,{color:selectedStore.primary_color||"#f4c400"}]}>CLICK-FOOD</Text></View>}
...
replace_once(old_header, new_header, "cabeçalho e categorias do cardápio")

old_segment = '''      <View style={styles.segment}>
...
new_segment = '''      {(selectedStore.clickfood_delivery_enabled||selectedStore.own_delivery_enabled||selectedStore.pickup_enabled)&&<View style={styles.segment}>
...
replace_once(old_segment, new_segment, "seletor de modalidade")
...
replace_once(
    ...
    "estilos de identidade e serviços",
)
...
print("Patch do Cliente aplicado com sucesso.")
