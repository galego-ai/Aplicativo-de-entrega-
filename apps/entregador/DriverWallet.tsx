...
export default function DriverWallet(){
 const[loading,setLoading]=useState(true);const[busy,setBusy]=useState(false);const[summary,setSummary]=useState<Summary>({availableBalance:0,payouts:[]});const[amount,setAmount]=useState("");const[pixKey,setPixKey]=useState("");const[message,setMessage]=useState("");
 ...
 async function load(){setLoading(true);const{data,error}=await supabase.functions.invoke("payout-action",{body:{action:"DRIVER_SUMMARY"}});if(error||data?.error){setMessage("Não foi possível consultar sua carteira agora.");setLoading(false);return;}setSummary({availableBalance:Number(data.availableBalance??0),payouts:(data.payouts??[]).map((p:any)=>({...p,amount:Number(p.amount)}))});setLoading(false);}
 useEffect(()=>{void load();},[]);
 ...
 async function requestPayout(){
  ...
  if(!pixKey.trim()){setMessage("Informe a chave PIX que receberá o repasse.");return;}
  Alert.alert("Solicitar repasse PIX",`Solicitar ${brl(parsed)} para a chave informada?`,[
   ...
   {text:"SOLICITAR",onPress:async()=>{
    ...
    if(error||data?.error){const code=data?.error;setMessage(code==="INSUFFICIENT_AVAILABLE_BALANCE"?"Seu saldo disponível mudou. Atualize e tente novamente.":"Não foi possível solicitar o repasse.");setBusy(false);return;}
    setAmount("");setPixKey("");setMessage("Repasse solicitado. A Matriz fará a análise antes do pagamento.");await load();setBusy(false);
   }},
  ]);
 }
