...
export default function ConfiguracaoPage(){
 ...
 return <main className="configPage"><div className="configShell"><header className="configHeader"><div><div className="logo"><span>CLICK</span>-FOOD</div><small>{store.name} • {store.role}</small><h1>Configuração da loja</h1><p>Identidade visual, dados comerciais, localização, atendimento e horários.</p></div><div style={{display:"flex",gap:8,flexWrap:"wrap"}}><a className="backLink" href="/produtos">Produtos</a><a className="backLink" href="/">← Voltar ao painel</a></div></header><StoreSettings storeId={store.id} role={store.role} onChanged={()=>void load()}/></div></main>;
}
