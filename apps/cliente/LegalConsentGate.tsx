...
export default function LegalConsentGate(){
  ...
  useEffect(()=>{void load();},[]);
  async function load(){
    const{data,error}=await supabase.functions.invoke("legal-consent",{body:{action:"STATUS",audience:"CUSTOMER",app:"CUSTOMER"}});
  ...
  async function accept(){
    ...
    setBusy(true);setMessage("");const{data,error}=await supabase.functions.invoke("legal-consent",{body:{action:"ACCEPT",audience:"CUSTOMER",app:"CUSTOMER",documentIds:ids}});setBusy(false);
  ...
  return <Modal visible={visible} animationType="slide" presentationStyle="fullScreen" onRequestClose={()=>{}}>
    <SafeAreaView style={styles.safe}><View style={styles.header}><Text style={styles.brand}><Text style={styles.yellow}>CLICK</Text>-FOOD</Text><Text style={styles.title}>Termos e Privacidade</Text><Text style={styles.subtitle}>Para continuar, leia e aceite a versão atual dos documentos.</Text></View>
