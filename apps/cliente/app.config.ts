...
export default ({ config }: ConfigContext): ExpoConfig => {
  ...
  if (process.env.EAS_BUILD === "true" && !mapsIsConfigured) {
    throw new Error(
      "GOOGLE_MAPS_ANDROID_API_KEY não configurada no ambiente EAS do CLICK-FOOD Cliente.",
    );
  }
  ...
  return {
    ...config,
    name: config.name ?? "CLICK-FOOD Cliente",
    slug: config.slug ?? "click-food-cliente",
    plugins,
    extra: {
      ...(config.extra ?? {}),
